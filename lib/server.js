...
var screenMirror = require('./screenMirrorServer');
...
Server.prototype.start = function(opts) {
  ...
  var screenMirrorPort = opts.screenMirrorPort || 7100;
  ...
  this.airTunesServer = rtsp.createServer(function(rtspConnection) {
    console.log(rtspConnection);
  });
  console.log('listening', airTunesPort);
  this.airTunesServer.listen(airTunesPort);

  this.screenMirrorServer = screenMirror.createServer(function(conn) {
    console.log(conn);
  });
  console.log('listening', screenMirrorPort);
  this.screenMirrorServer.listen(screenMirrorPort);
};
...
Server.prototype._onPostFpSetup = function(req, res) {
  ...
  var body;
  if (req.headers['content-length'] == 16) {
    body = new Buffer('', 'hex');
  } else {
    body = new Buffer('46504c5903010400000000142982693bac00eda5b4c38494c7c36ede55601478', 'hex');
  }
  ...
  var str =
    'RTSP/1.0 200 OK\r\n'
      ...
      + 'CSeq: 0\r\n'
      + '\r\n';
  var data = Buffer.concat([new Buffer(str), body]);
  res.write(data);
};
